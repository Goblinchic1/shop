...
<div class="product-card flex flex-col md:flex-row rounded-3xl bg-card">
    ...
    <div class="grow flex flex-col py-8 px-6 md:px-8">
        <h3 class="text-sm lg:text-md font-black"><a href="{{ route('product', ['product' => $item]) }}"
                                                     class="inline-block text-white hover:text-pink">{{ $item->title }}</a>
        </h3>
        @if($item->json_properties)
            <ul class="space-y-1 mt-4 text-xxs">
                @foreach($item->json_properties as $propertyName => $propertyValue)
                    <li class="flex justify-between text-body"><strong>{{ $propertyName }}</strong> {{ $propertyValue }}
                    </li>
                @endforeach
            </ul>
        @endif
